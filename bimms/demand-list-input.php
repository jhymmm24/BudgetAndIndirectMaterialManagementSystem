...
// Calculate the previous 3 months
$previousMonths = [];
for ($i = 2; $i >= 0; $i--) {
    $prevMonth = ($currentMonth - $i - 1 + 12) % 12; // Corrected to properly wrap months
    if ($prevMonth == 0) {
        $prevMonth = 12; // Adjust for December
    }
    $year = ($prevMonth > $currentMonth) ? $currentYear - 1 : $currentYear; 
    $previousMonths[] = "{$monthNames[$prevMonth - 1]} $year"; 
}

// Create an array for the next 12 months after the current month
// Create an array for the next 12 months after the current month
$nextMonths = [];
for ($i = 0; $i < 12; $i++) {
    $nextMonth = ($currentMonth + $i) % 12; // Adjust for 1-12 range
    if ($nextMonth == 0) {
        $nextMonth = 12; // Adjust for December
    }
    
    // Adjust year calculation
    $year = $currentYear + floor(($currentMonth + $i - 1) / 12); 
    $nextMonths[] = "{$monthNames[$nextMonth - 1]} $year"; 
}
// Combine previous 3 months with the next 12 months
$allMonths = array_merge($previousMonths, $nextMonths);
...
<thead>
  <tr>
      ...
      <th style="text-align: center;">Calculated Availability Date</th>
      <th style="text-align: center;">Delivery Date</th>
      <?php foreach ($allMonths as $month): ?>
          <th style="text-align: center;"><?= htmlspecialchars($month) ?></th>
      <?php endforeach; ?>
  </tr>
</thead>
<tbody>
...
$monthCount = 0; // Counter for the months
$disabledColumns = []; // Array to store disabled columns per row

foreach ($allMonths as $month) {
    // Convert month name to a key format
    $monthKey = strtolower(str_replace(' ', '_', trim($month)));

    // Parse the current lead date
    $currentLeadDateITEM = $currentLeadDate->format('Y-m-d');
    $currentMonth = $currentLeadDate->format('F');
    $currentYear = $currentLeadDate->format('Y');

    // Create a DateTime object from the current month in the loop
    $monthDate = DateTime::createFromFormat('F Y', $month);

    // Check if the month in the loop is before or equal to the current lead date
    $isBeforeOrEqualLeadDate = $monthDate <= $currentLeadDate;

    // Set the disabled attribute based on the condition
    $disabledAttribute = ($isBeforeOrEqualLeadDate) ? 'disabled' : '';

    // Add the disabled monthKey to the array if disabled
    if ($disabledAttribute === 'disabled') {
        $disabledColumns[] = $monthKey;
    }

    // Determine the style for the input field
    if ($monthCount < 3) {
        $style = 'background-color: yellow;'; // Color for the first 3 months
    } else {
        $style = ($isBeforeOrEqualLeadDate) ? 'background-color: red;' : ''; // Color for other months
    }

    // Get existing value or default to 0
    $value = isset($row[$monthKey]) ? $row[$monthKey] : 0;

    // Output the <td> with the disabled input based on the condition
    echo '<td style="text-align: center;">
        <input type="number" class="month-input" data-month="' . htmlspecialchars($monthKey) . '" name="months[' . htmlspecialchars($monthKey) . '][' . $item_code2 . ']" value="' . htmlspecialchars($value) . '" ' . $disabledAttribute . ' style="' . $style . '">
    </td>';

    $monthCount++; // Increment the month counter
}

// // Output the disabled columns for the current row
// if (!empty($disabledColumns)) {
//     echo 'Disabled columns for this row: ' . implode(', ', $disabledColumns) . '<br>';
// }

// Reset the disabled columns array for the next row
$disabledColumns = [];
          ...
          <div class="row">
            <div class="col-md-6 col-lg-6">
                  <div class="form-group">
                           ...
                           // Perform the SQL query to fetch the options
                           $query = "SELECT Distinct main_pic FROM tbl_costcenter WHERE section = '$section'";
                        ...
                        <div class="form-group">
                              ...
                              // Perform the SQL query to fetch the options
                              $query = "SELECT Distinct section_spv FROM tbl_costcenter WHERE section = '$section'";
                              $result = pg_query($query);
                              ...
                              <div class="form-group">
                                    ...
                                    // Perform the SQL query to fetch the options
                                    $query = "SELECT Distinct section_manager FROM tbl_costcenter WHERE section = '$section'";
                                    $result = pg_query($query);
...
function confirm_submit() {
    ...
    if (count <= 0) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                [select4, select5, select6, select7, select8].forEach((select, index) => {
                    var selectInput = document.createElement('input');
                    selectInput.type = 'hidden';
                    selectInput.name = ['sectionBudgetController', 'sectionSupervisor', 'sectionManager', 'peSectionSupervisor', 'peSectionManager'][index];
                    selectInput.value = select.value;
                    form.appendChild(selectInput);
                });

                // Collect all month input fields and add them to the form
                ...
                monthInputs.forEach(function(monthInput) {
                    ...
                    clonedInput.type = 'hidden'; // Use hidden inputs for submission
                    clonedInput.name = monthInput.name; // Use the month key as name
                    clonedInput.value = monthInput.value; // Use the input's value
                ...
                .then(response => {
                    ...
                    return response.text(); // or response.json() if you're expecting a JSON response
                })
                .then(data => {
                // Assuming the server sends a success response
                Swal.fire({
                    title: 'Success!',
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = 'demand-list-input.php'; // Replace with your page URL
                        // Or, if you want to refresh the current page:
                        // location.reload();
                    }
                });
            })
  ...
  // Restore the cost center after page reload
  window.onload = function() {
    var storedCostCenter = localStorage.getItem('cost_center');
    if (storedCostCenter) {
      document.getElementById('cost_center').value = storedCostCenter;
      document.getElementById('cost_center').disabled = true;
    }
...
// <script>
//     $(document).ready(function () {
//         // Trigger when a different option is selected
//         $('#cost_center').change(function () {
//             var selectedCostCenter = $(this).val();
//             var found = false;

//             // Loop through table rows to check for the selected value in column 8
//             $('#example25 tr').each(function () {
//                 var columnValue = $(this).find('td:eq(8)').text(); // Column 8 is index 7

//                 if (selectedCostCenter === columnValue) {
//                     found = true;
//                     return false; // Break loop if found
//                 }
//             });

//             // If the value is not found in column 8, show SweetAlert and display the selected value
//             if (!found && selectedCostCenter !== "") {
//                 Swal.fire({
//                     icon: 'error',
//                     title: 'Value not found!',
//                     text: 'The selected cost center "' + selectedCostCenter + '" does not exist in the previous selected cost center in the current transaction.',
//                 });
//             }
//         });
//     });
