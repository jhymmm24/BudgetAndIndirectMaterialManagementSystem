                     ...
                     <div class="row column_title">
                        <div class="col-md-12">
                           <div class="page_title">
                              <h2 style="margin-top:20px;">Stock Information Setting</h2>
                ...
                <tbody>
                    <?php
                        ...
                        while ($row = pg_fetch_assoc($stmt3)) {
                            ...
                            echo '<tr id="row-' . $row['id'] . '" style="' . $background_color . '">
                            <td style="font-size: 12px; text-align: center;">
                                <button type="button" class="btn-edit" onclick="editRow(' . $row['id'] . ')">Edit</button>
...
function editRow(rowId) {
    ...
    // Loop through each cell to make it editable
    for (var i = 1; i < cells.length; i++) { // start from 1 to skip the first cell (buttons)
        var cell = cells[i];
        ...
        input.style.width = '100%'; // Make input take the full width
        cell.innerHTML = ''; // Clear the cell
        cell.appendChild(input); // Add the input to the cell
    }

    // Optionally, change the Edit button to Save button
    var editButton = row.getElementsByClassName('btn-edit')[0];
    editButton.innerText = 'Save';
    editButton.setAttribute('onclick', 'saveRow(' + rowId + ')'); // Change function to save
}
...
function saveRow(rowId) {
    ...
    // Loop through each cell to save the values
    for (var i = 1; i < cells.length; i++) {
        var cell = cells[i];
        var input = cell.getElementsByTagName('input')[0];

        if (input) {
            cell.innerText = input.value; // Save the input value back to the cell
        }
    }
    ...
    editButton.setAttribute('onclick', 'editRow(' + rowId + ')'); // Change function back to edit
}
</script>
        </div>
    </div>
            ...
            initComplete: function () {
                ...
                $('#select4-warehouse').on('change', function() {
                    ...
                    if (val === "F1- Excess Item,F1-3F Stockroom") {
                        ...
                        table.column(1).search('F1- Excess Item|F1-3F Stockroom', true, false).draw();
                    }else if(val === "F2 PnJ_Excess Item,F2-2F Stockroom"){
                         // Perform OR search for "All Warehouse"
                         table.column(1).search('F2 PnJ_Excess Item|F2-2F Stockroom', true, false).draw();
                    }
                    else if(val === "F2 PIM_Excess Item,F2-2F Stockroom"){
                         // Perform OR search for "All Warehouse"
                         table.column(1).search('F2 PIM_Excess Item|F2-2F Stockroom', true, false).draw();
                    } else {
